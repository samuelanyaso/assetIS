...
where the maximum is over all $`2^M - 1`$ non-empty subsets $`A`$ of
$`M`$ studies (or cell types). Evaluating its significance requires an
extreme tail probability, which is computationally intractable by naive
Monte Carlo. The IS estimators in **assetIS** use exponential tilting to

...
Z_{\text{ASSET}} = \max_{A \neq \varnothing} |Z_A|,
```
where the maximum is taken over all $`2^M - 1`$ non-empty subsets $`A`$
of $`M`$ studies (or cell types). Computing the significance of
...
$`n_m^{\text{eff}} = n_m \phi_m(1-\phi_m)`$, where $`\phi_m`$ is the
case fraction in study $`m`$.
...
We consider $`C = 5`$ cell types, $`N = 100`$ subjects, and MAF
$`f = 0.15`$.
...
print(res_nncorr)
#>          b     p.est        p.se   p.lower   p.upper b_anchor    K
#> 1 1.786694 0.2796077 0.008487158 0.2629729 0.2962426 1.786694 2000
```
...
$`K \geq 5 \times 10^4`$ for the non-Gaussian estimators with modest
$`C`$ ($`\leq 10`$).
